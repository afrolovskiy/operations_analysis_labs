...
public class ExtendMatrix extends Matrix
{
    ...
    private String printElementMarkToString(int indexRow, int indexColumn)
    {
        if (hasElemMark(indexRow, indexColumn, '*'))
            ...
            return "\'";
        else if (hasElemMark(indexRow, indexColumn, (char)8734))
            return "" + (char)8734;
        else
            return " ";  
    }
    ...
    public ExtendMatrix copy()
    {
        int degree = this.getDegree();
        ExtendMatrix resMatrix = new ExtendMatrix(degree);
        resMatrix.values = this.copyValues();
        resMatrix.markColumns = this.copyMarkColums();
        resMatrix.markRows = this.copyMarkRows();
        resMatrix.marks = this.copyMarks();
        return resMatrix;
    }
    
    protected byte[] copyMarkColums()
    {
        return copyByteMassiv(this.markColumns);
        
    }
    
    private byte[] copyByteMassiv(byte[] massiv)
    {
        int size = massiv.length;
        byte[] res = new byte[size];
        System.arraycopy(massiv, 0, res, 0, size);
        return res;
    }
    
    protected byte[] copyMarkRows()
    {
        return copyByteMassiv(this.markRows);
    }
    
    protected char[][] copyMarks()
    {
        int size = this.getDegree();
        char[][] res = new char[size][size];
        for (int  i = 0; i < size; i++)
            for (int j = 0; j < size; j++)
                res[i][j] = this.marks[i][j];
        return res;
    }
